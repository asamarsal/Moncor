...
function imports(i){let p;if(i.startsWith('@openzeppelin/'))p=path.join(root,'node_modules',i);else if(i.startsWith('forge-std/'))p=path.join(root,'lib','forge-std','src',i.slice('forge-std/'.length));else p=path.join(root,i);try{return{contents:fs.readFileSync(p,'utf8')}}catch{return{error:`Import not found: ${i}`}}}
